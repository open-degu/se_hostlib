...
U16 SCP_GP_ExternalAuthenticate(ChannelId_t channelId, U8* hostCryptogram)
{
    ...
    */
    ...
    if (st != SMCOM_OK)
    {
        EPRINTF("SCP_GP_ExternalAuthenticate %X\n", st);
    ...
    U8 *seqCounter, U16 *pSeqCounterLen)
{
    ...
    if (st != SMCOM_OK)
    {
        EPRINTF("SCP_GP_InitializeUpdate. Failure on communication Link (0x%04X)\n", st);
        return (U16)st;
    }
    ...
    if ((responseLen != iuResponseLenSmall) && (responseLen != iuResponseLenBig))
    {
        ...
        EPRINTF("Unexpected amount of data returned: %d\n", responseLen);
        return SCP_FAIL;
    }
...
U16 SCP02_GP_ExternalAuthenticate(ChannelId_t channelId, U8* hostCryptogram)
{
    ...
    if (st != SMCOM_OK) {
        EPRINTF("SCP01_GP_ExternalAuthenticate %X\n", st);
