...
#elif defined(ZEPHYR) && !defined(CONFIG_POSIX_API)
...
void axSummarizeMeasurement(FILE *fHandle, char *szMessage, long *msArray, int nMeasurement, int reportMode)
{
    ...
    for (i=0; i<nMeasurement; i++)
    {
        if (fReportVerbose)
        {
            for (nOut=0; nOut<nHandle; nOut++)
            {
                fprintf(fHandleArray[nOut], "%s: %ld ms\n", szMessage, msArray[i]);
            }
        }
        averaged += msArray[i];
    ...
    for (nOut=0; nOut<nHandle; nOut++)
    {
        ...
        fprintf(fHandleArray[nOut], "\tMinimum: %ld ms\n", minValue);
        fprintf(fHandleArray[nOut], "\tMaximum: %ld ms\n", maxValue);
    }
}
